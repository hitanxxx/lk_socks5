...
status systime_update(void) {
    ...
    sprintf((char *)time_str_gmt, "%s, %02d %s %04d %02d:%02d:%02d GMT",
    ...
    sprintf(
        (char *)time_str_log, "%04d/%02d/%02d %02d:%02d:%02d.%03d",
        (unsigned int)(local.tm_year % 1000 + 2000),
        (unsigned int)(local.tm_mon % 100), (unsigned int)(local.tm_mday % 100),
        (unsigned int)(local.tm_hour % 100), (unsigned int)(local.tm_min % 100),
        (unsigned int)(local.tm_sec % 100), msec);
    return 0;
}
