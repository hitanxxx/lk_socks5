...
static int config_parse(char *str) {
    ...
    if (root) {
        ...
        if (ssl_crt_path)
            strncpy(g_config.ssl_crt_path, cJSON_GetStringValue(ssl_crt_path),
                    sizeof(g_config.ssl_crt_path) - 1);
        if (ssl_key_path)
            strncpy(g_config.ssl_key_path, cJSON_GetStringValue(ssl_key_path),
                    sizeof(g_config.ssl_key_path) - 1);
        ...
        cJSON *s5_serv_auth_path =
            cJSON_GetObjectItem(root, "s5_serv_auth_path");
        ...
        if (s5_serv_auth_path)
            strncpy(g_config.s5_serv_auth_path,
                    cJSON_GetStringValue(s5_serv_auth_path),
                    sizeof(g_config.s5_serv_auth_path) - 1);
        if (s5_serv_gw)
            strncpy(g_config.s5_serv_gw, cJSON_GetStringValue(s5_serv_gw),
                    sizeof(g_config.s5_serv_gw) - 1);
        /// dbg("s5_mode: [%d]\n", g_config.s5_mode);
        ...
        cJSON *s5_local_serv_port =
            cJSON_GetObjectItem(root, "s5_local_serv_port");
        ...
        if (s5_local_serv_ip)
            strncpy(g_config.s5_local_serv_ip,
                    cJSON_GetStringValue(s5_local_serv_ip),
                    sizeof(g_config.s5_local_serv_ip) - 1);
        if (s5_local_auth)
            strncpy(g_config.s5_local_auth, cJSON_GetStringValue(s5_local_auth),
                    sizeof(g_config.s5_local_auth) - 1);
        ...
        if (http_arr) {
            for (i = 0; i < cJSON_GetArraySize(http_arr); i++)
                g_config.http_arr[i] =
                    cJSON_GetArrayItem(http_arr, i)->valueint;
        }
        if (https_arr) {
            for (i = 0; i < cJSON_GetArraySize(https_arr); i++)
                g_config.https_arr[i] =
                    cJSON_GetArrayItem(https_arr, i)->valueint;
        }
        if (http_home)
            strncpy(g_config.http_home, cJSON_GetStringValue(http_home),
                    sizeof(g_config.http_home) - 1);
        if (http_index)
            strncpy(g_config.http_index, cJSON_GetStringValue(http_index),
                    sizeof(g_config.http_index) - 1);
