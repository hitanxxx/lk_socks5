...
{
    if(g_event_ctx->epfd) close(g_event_ctx->epfd);    
    if(g_event_ctx->epev) mem_pool_free( g_event_ctx->epev);
    return 0;
}

static int ev_epoll_opt(con_t * c, int want_opt)
{
    if(c->ev->opt != want_opt) { ///want type not same as record type
        if(want_opt == EV_NONE) {
...
#endif    
    if(g_event_handler.init) g_event_handler.init();
