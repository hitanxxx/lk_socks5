...
static int ev_epoll_init() {
    g_event_ctx->epev =
        mem_pool_alloc(sizeof(struct epoll_event) * MAX_NET_CON);
...
int ev_epoll_loop(time_t msec) {
    ...
    int all = epoll_wait(g_event_ctx->epfd, g_event_ctx->epev, MAX_NET_CON,
                         (int)msec);
    if (all <= 0) {
        if (all == 0) {
            return (msec == -1) ? -1 : -11;
...
int ev_select_loop(time_t msec) {
    ...
    memcpy(
        &rfds, &g_event_ctx->rfds,
        sizeof(fd_set)); /// select return will be change read fds and write fds
    memcpy(&wfds, &g_event_ctx->wfds, sizeof(fd_set));
    actall =
        select(fdmax + 1, &rfds, &wfds, NULL, (msec != -1) ? &wait_tm : NULL);
    if (actall <= 0) {
        if (actall == 0) {
            return (msec == -1) ? -1 : -11;
        }
...
int ev_loop(time_t msec) {
    ...
    g_event_ctx->evn = 0;
    if (g_event_handler.run)
        g_event_handler.run(msec);
    systime_update();
