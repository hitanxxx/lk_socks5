...
/// @brief create a ringbuffer, all space is size. ringbuffer means memory
/// buffer
/// @param rb
/// @param size
/// @return
...
int rb_full(ringbuffer_t *rb) { return (rb->space == rb->datan ? 1 : 0); }

int rb_empty(ringbuffer_t *rb) { return (rb->datan == 0 ? 1 : 0); }
...
int ringbuffer_pull(ringbuffer_t *rb, int datan, unsigned char *data,
                    int *outn) {
    pthread_mutex_lock(&rb->lock);
    if (rb_empty(rb)) {
        pthread_mutex_unlock(&rb->lock);
    ...
    if (rb->pos < rb->last) {
        /// start-----pos-----last-----end
        int pulln =
            (datan >= (rb->last - rb->pos) ? (rb->last - rb->pos) : datan);
...
int ringbuffer_getlen(ringbuffer_t *rb) { return (rb->datan); }

int ringbuffer_getfree(ringbuffer_t *rb) { return (rb->space - rb->datan); }
