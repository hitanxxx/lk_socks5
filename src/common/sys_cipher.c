...
int sys_aesgcm_enc(sys_cipher_t *ctx, unsigned char *in, int inn,
    ...
    outl += len;
    schk(1 == EVP_CIPHER_CTX_ctrl(ctx->ctx, EVP_CTRL_GCM_GET_TAG, 16, tag),
         return -1);
    return outl;
}
...
int sys_aesgcm_dec(sys_cipher_t *ctx, unsigned char *in, int inn,
    ...
    outl += len;
    schk(1 == EVP_CIPHER_CTX_ctrl(ctx->ctx, EVP_CTRL_GCM_SET_TAG, 16, tag),
         return -1);
...
int sys_aesgcm_ctx_init(sys_cipher_t **sctx, int typ) {
    ...
    do {
        ...
        if (typ == 0) {
            schk(1 == EVP_EncryptInit_ex(ctx->ctx, EVP_aes_256_gcm(), NULL,
                                         NULL, NULL),
                 break);
            schk(1 == EVP_CIPHER_CTX_ctrl(ctx->ctx, EVP_CTRL_GCM_SET_IVLEN, 16,
                                          NULL),
                 break);
            schk(1 == EVP_EncryptInit_ex(ctx->ctx, NULL, NULL,
                                         (unsigned char *)key,
                                         (unsigned char *)iv),
                 break);
        } else {
            schk(1 == EVP_DecryptInit_ex(ctx->ctx, EVP_aes_256_gcm(), NULL,
                                         NULL, NULL),
                 break);
            schk(1 == EVP_CIPHER_CTX_ctrl(ctx->ctx, EVP_CTRL_GCM_SET_IVLEN, 16,
                                          NULL),
                 break);
            schk(1 == EVP_DecryptInit_ex(ctx->ctx, NULL, NULL,
                                         (unsigned char *)key,
                                         (unsigned char *)iv),
                 break);
        }
...
int aes_cfb_encrypt(unsigned char *in, int inlen, unsigned char *out) {
    ...
    if (ctx) {
        ...
        EVP_EncryptInit_ex(ctx, EVP_aes_256_cfb8(), NULL, (unsigned char *)key,
                           (unsigned char *)iv);
...
int aes_cfb_decrypt(unsigned char *in, int inlen, unsigned char *out) {
    ...
    if (ctx) {
        EVP_CIPHER_CTX_init(ctx);
        EVP_DecryptInit_ex(ctx, EVP_aes_256_cfb8(), NULL, (unsigned char *)key,
                           (unsigned char *)iv);
        do {
