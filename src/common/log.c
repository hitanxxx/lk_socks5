...
static string_t levels[] = {string("[error]"), string("[debug]"),
                            string("[info]")};

static int log_format_prefix(log_content_t *ctx) {
    ctx->pos +=
        snprintf(ctx->pos, (ctx->last - ctx->pos) - 1, "%.*s %s <%s:%d> ",
                 levels[ctx->level].len, levels[ctx->level].data, systime_log(),
                 ctx->func, ctx->line);
    return 0;
}
...
static int log_format_text(log_content_t *ctx) {
    ...
    ctx->pos += vsnprintf(ctx->pos, (ctx->last - ctx->pos) - 1, ctx->args,
                          ctx->args_list);
...
int log_init(void) {
    do {
        ...
        log_ctx->log_fd_main =
            open(S5_PATH_LOG_FILE_MAIN, O_CREAT | O_RDWR | O_APPEND, 0644);
        schk(log_ctx->log_fd_main > 0, break);

        log_ctx->log_fd_access =
            open(S5_PATH_LOG_FILE_ACCESS, O_CREAT | O_RDWR | O_APPEND, 0644);
