...
int net_socket_reuseport(int fd) {
    ...
    return setsockopt(fd, SOL_SOCKET, SO_REUSEPORT,
                      (const void *)&tcp_reuseport, sizeof(int));
}

int net_socket_reuseaddr(int fd) {
    int tcp_reuseaddr = 1;
    return setsockopt(fd, SOL_SOCKET, SO_REUSEADDR,
                      (const void *)&tcp_reuseaddr, sizeof(int));
}

int net_socket_fastopen(int fd) {
#if 0
    int  tcp_fastopen = 1;
    return setsockopt(fd, IPPROTO_TCP,
        TCP_FASTOPEN, (const void *) &tcp_fastopen, sizeof(tcp_fastopen));
#endif
    return 0;
}
...
int net_socket_nodelay(int fd) {
    int tcp_nodelay = 1;
    return setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const void *)&tcp_nodelay,
                      sizeof(int));
}
...
int net_socket_nopush(int fd) {
    ...
    int tcp_cork = 1;
    return setsockopt(fd, IPPROTO_TCP,
        TCP_CORK, (const void *) &tcp_cork, sizeof(int));
#endif
    return 0;
}
...
int net_socket_lowat_send(int fd) {
    ...
    return setsockopt(fd, SOL_SOCKET,
        SO_SNDLOWAT, (const void*)&lowat, sizeof(int));
#endif
    return 0;
}
...
int net_socket_check_status(int fd) {
    ...
    schk(getsockopt(fd, SOL_SOCKET, SO_ERROR, (void *)&err, &errn) != -1,
         return -1);
    if (err != 0)
        return -1;
    return 0;
...
int net_connect(con_t *c, struct sockaddr_in *addr) {
    ...
    /// check writeable event for result of connect
    ev_opt(c, EV_W);
...
int net_accept(con_t *c) {
    ...
    for (;;) {
        ...
        schk(net_socket_nbio(cc->fd) == 0, {
            net_free(cc);
            return -1;
        });
        schk(net_socket_nodelay(cc->fd) == 0, {
            net_free(cc);
            return -1;
        });
        schk(net_socket_fastopen(cc->fd) == 0, {
            net_free(cc);
            return -1;
        });
        schk(net_socket_lowat_send(cc->fd) == 0, {
            net_free(cc);
            return -1;
        });
