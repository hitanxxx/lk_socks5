...
status net_accept( event_t * ev )
{
	...
	while( 1 ) 
	{
		...
		if( OK != net_socket_nbio( client_fd ) ) 
		{
			err("socket set nonblock failed\n" );
			net_free( client_con );
			return ERROR;
		}
		if( OK != net_socket_nodelay( client_fd ) )
		{
		...
		if( OK != net_socket_fastopen( client_fd ) )
		{
			err("socket set fastopen failed\n");
		}
		if( OK != net_socket_lowat_send( client_fd ) )
		{
		...
		event_opt( client_con->event, client_fd, EV_R );
