...
static int webser_api_find(webser_t *web) {
    ...
    for (i = 0; i < g_web_ctx->g_api_list->elem_num; i++) {
        ...
        if (strncmp(api_entry->key, (char *)web->webreq->uri.data,
                    web->webreq->uri.len)) {
            continue;
        }
...
static void web_release_c(void *data) {
    webser_t *websrv = data;
...
static int webser_keepalive(con_t *c) {
    ...
    /// process net con meta remian data
...
static int webser_rsp_hdrp(con_t *c, const char *str, ...) {
    ...
    if (!webc->rsp_hdr) {
        schk(0 == meta_alloc(&webc->rsp_hdr, 4096), {
            net_free(c);
            return -1;
        });
    }
    ...
    schk(0 == meta_alloc(&n, 4096), {
        net_free(c);
        return -1;
    });
    schk(0 == meta_pdata(n, buf, strlen(buf)), {
        net_free(c);
        return -1;
    });
    p->next = n;
...
static int webser_req_router(con_t *c) {
    ...
    if (rc == 0) {
        ...
        char uri[1024] = {0};
        memcpy(uri, webc->webreq->uri.data,
               webc->webreq->uri.len > 1024 ? 1024 : webc->webreq->uri.len);
        schk(!strstr(uri, "../"), {
            net_free(c);
            return -1;
        });
...
static int webser_start(con_t *c) {
    ...
    /// it
    schk(webc->webreq = web_req_alloc(), {
        net_free(c);
        return -1;
    });
...
static int webser_init_mimehash(void) {
    ...
    ezhash_add(g_web_ctx->mime_hash, ".html", strlen(".html"), "text/html",
               strlen("text/html"));
    ...
    ezhash_add(g_web_ctx->mime_hash, ".png", strlen(".png"), "image/png",
               strlen("image/png"));

    ezhash_add(g_web_ctx->mime_hash, ".jpg", strlen(".jpg"), "image/jpeg",
               strlen("image/jpeg"));

    ezhash_add(g_web_ctx->mime_hash, ".jpeg", strlen(".jpeg"), "image/jpeg",
               strlen("image/jpeg"));

    ezhash_add(g_web_ctx->mime_hash, ".gif", strlen(".gif"), "image/gif",
               strlen("image/gif"));

    ezhash_add(g_web_ctx->mime_hash, ".ico", strlen(".ico"), "image/x-icon",
               strlen("image/x-icon"));

    ezhash_add(g_web_ctx->mime_hash, ".css", strlen(".css"), "text/css",
               strlen("text/css"));

    ezhash_add(g_web_ctx->mime_hash, ".txt", strlen(".txt"), "text/plain",
               strlen("text/plain"));

    ezhash_add(g_web_ctx->mime_hash, ".htm", strlen(".htm"), "text/html",
               strlen("text/html"));

    ezhash_add(g_web_ctx->mime_hash, ".mp3", strlen(".mp3"), "audio/mpeg",
               strlen("audio/mpeg"));

    ezhash_add(g_web_ctx->mime_hash, ".m3u8", strlen(".m3u8"),
               "application/x-mpegURL", strlen("application/x-mpegURL"));

    ezhash_add(g_web_ctx->mime_hash, ".ts", strlen(".ts"), "video/MP2T",
               strlen("video/MP2T"));
    return 0;
}
...
int webser_init(void) {
    ...
    do {
        ...
        schk(mem_arr_create(&g_web_ctx->g_api_list, sizeof(webser_api_t)) == 0,
             break);
