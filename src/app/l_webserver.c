...
static status webser_procotol_route( event_t * ev )
{
    connection_t * c = ev->data;
    ...
    do
    {
        ...
        ev->read_pt = socks5_server_secret_mode_start;
        return ev->read_pt( ev );
    } while(0);
...
static status webser_accept_callback_ssl( event_t * ev )
{
    ...
    ev->write_pt 	    = NULL;
    ev->read_pt  	    = ( conf.socks5_mode == SOCKS5_SERVER_SECRET) ? webser_procotol_route : webser_cycle_init;
    return ev->read_pt( ev );
}
