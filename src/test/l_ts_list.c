...
static void ts_list_create( void )
{
	...
	rc = mem_list_create( &list, sizeof(string_t) );
	t_assert( rc == OK );
}
// -----------------------------
static void ts_list_push( void )
{
	...
	a = mem_list_push( list );
	t_assert( a );
	
	b = mem_list_push( list );
	t_assert( b );
	
	c = mem_list_push( list );
	t_assert( c );
...
static void ts_list_get( void )
{
	...
	str = mem_list_get( list, 1 );
	t_assert( str );
	t_assert( str->data == x.data );
	t_assert( str->len == x.len );
	
	str = (string_t*)mem_list_get( list, 2 );
	t_assert( str );
	t_assert( str->data == y.data );
	t_assert( str->len == y.len );
	
	str = (string_t*)mem_list_get( list, 3 );
	t_assert( str );
	t_assert( str->data == z.data );
	t_assert( str->len == z.len );
	
	str = (string_t*)mem_list_get( list, 4 );
	t_assert( !str );
	
	str = (string_t*)mem_list_get( list, 0 );
	t_assert( !str );
}
// ---------------------------
...
static void ts_list_get_null( void )
{
	...
	rc = mem_list_create( &list, sizeof(char) );
	t_assert( rc == OK );  
	
	c = mem_list_get( list, 1 );
	t_assert( c == NULL );
